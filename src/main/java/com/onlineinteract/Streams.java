...
/**
 * The Java Streams API contains a set of non-terminal operations: filter, map,
 * flatMap, distinct, limit, peek
 * <p>
 * The Java Streams API contains a set of terminal operations: anyMatch/all
 * Match/noneMatch, collect, count, findAny/findAll, forEach, min/max, reduce,
 * toArray
 * <p>
 ...
 * 
 * References: https://www.youtube.com/watch?v=bcrl-GL0vV4
...
//		streams.samples2();
	...
	public void samples2() throws IOException, URISyntaxException {
		// 1. Integer Stream.
		...
		Stream.of("Gary", "Derek", "Alicia").filter(x -> x.startsWith("Ga")).sorted().findFirst()
				.ifPresent(System.out::println);
		System.out.println();

		// 5. Stream from array, sort, filer and print.
		String[] names = { "Gary", "Derek", "Alicia", "Nadine", "George" };
		Arrays.stream(names).filter(x -> x.startsWith("G")).sorted().forEach(System.out::println);
		System.out.println();
		...
		Stream.of("Gary", "Derek", "Alicia").filter(x -> x.length() > 4).sorted().forEach(System.out::println);
		System.out.println();
		...
		bands.sorted().filter(x -> x.length() < 14).forEach(System.out::println);
		bands.close();
		System.out.println();

		// 9. Stream rows from text file and save to list.
		List<String> bands2 = Files.lines(path).filter(x -> x.contains("ovi")).collect(Collectors.toList());
